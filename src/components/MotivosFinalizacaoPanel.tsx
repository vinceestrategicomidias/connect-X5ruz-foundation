...
import { Plus, Pencil, Trash2, CheckCircle2, Building, LayoutGrid } from "lucide-react";
...
export const MotivosFinalizacaoPanel = () => {
    ...
    const motivosComRelacoes: Motivo[] = await Promise.all(
      motivosData.map(async (m: any) => {
        const { data: setoresRel } = await supabase
          .from("motivos_finalizacao_setores")
          .select("setor_id")
          .eq("motivo_id", m.id);
        const { data: unidadesRel } = await supabase
          .from("motivos_finalizacao_unidades")
          .select("unidade_id")
          .eq("motivo_id", m.id);
        return {
          ...m,
  ...
  const handleSalvar = async () => {
    ...
    try {
      ...
      if (editingMotivo) {
        ...
        await supabase.from("motivos_finalizacao_setores").delete().eq("motivo_id", motivoId);
        await supabase.from("motivos_finalizacao_unidades").delete().eq("motivo_id", motivoId);
      } else {
      ...
      if (setoresSelecionados.length > 0) {
        await supabase.from("motivos_finalizacao_setores").insert(
          setoresSelecionados.map((setor_id) => ({ motivo_id: motivoId, setor_id }))
        );
      }
      if (unidadesSelecionadas.length > 0) {
        await supabase.from("motivos_finalizacao_unidades").insert(
  ...
  const handleToggleAtivo = async (motivo: Motivo) => {
    await supabase.from("motivos_finalizacao").update({ ativo: !motivo.ativo }).eq("id", motivo.id);
    fetchMotivos();
  };

  const handleDelete = async (id: string) => {
    await supabase.from("motivos_finalizacao").delete().eq("id", id);
  ...
  const toggleSetor = (id: string) => {
    setSetoresSelecionados((prev) => prev.includes(id) ? prev.filter((s) => s !== id) : [...prev, id]);
  };
  const toggleUnidade = (id: string) => {
    setUnidadesSelecionadas((prev) => prev.includes(id) ? prev.filter((u) => u !== id) : [...prev, id]);
  };
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Cadastre os motivos disponíveis ao finalizar atendimentos
          </p>
        </div>
                  ...
                  <div className="flex flex-wrap gap-1">
                    {motivo.setores.map((sid) => (
                      <Badge key={sid} variant="outline" className="text-[10px] gap-1">
                        <LayoutGrid className="h-3 w-3" /> {getSetorNome(sid)}
                      </Badge>
                    ))}
                    {motivo.unidades.map((uid) => (
                      <Badge key={uid} variant="outline" className="text-[10px] gap-1">
                        <Building className="h-3 w-3" /> {getUnidadeNome(uid)}
                      </Badge>
                    ))}
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>{editingMotivo ? "Editar Motivo" : "Novo Motivo de Finalização"}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <div className="space-y-1.5">
              <Label className="text-xs">Nome *</Label>
              <Input value={nome} onChange={(e) => setNome(e.target.value)} placeholder="Ex: Vendido" />
            </div>
            <div className="space-y-1.5">
